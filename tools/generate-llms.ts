...
function extractRoutes(appJsxPath: string): RouteMap {
  ...
  try {
    ...
    for (const match of routeMatches) {
      ...
      if (elementMatch) {
        ...
        if (routePath) routes.set(componentName, routePath);
      }
    }
    return routes;
  } catch (error) {
...
function extractHelmetData(content: string, filePath: string, routes: RouteMap): PageData | null {
...
function processPageFile(filePath: string, routes: RouteMap): PageData | null {
  try {
